...
class Table extends React.Component<TableProps, TableState> {
        ...
        return (
            <div className="cr-table-container">
                <div className={cn}>
                    ...
                    {filters.length ? <div className="cr-table__filters">{this.renderFilters()}</div> : null}
                </div>
                <FilterInput
    ...
    private renderTitleComponent() {
        ...
        return <div className={'cr-title-component'}>{titleComponent}<span className="star"/><img className="move" src={move}/></div>;
    }
    ...
    private renderBody(rows: CellData[][], rowKeyIndex: number | undefined) {
        ...
        const dataToBeMapped = resultData || rows;
        const keyValue = 'rowKeyIndex';
        const rowElements = dataToBeMapped.map((r, i) => {
            const rowKey = String((rowKeyIndex !== undefined) ? r[keyValue] : i);
            return (
                <tr
...
// WEBPACK FOOTER //
// src/drone/src/src/custom/components/Table/index.tsx
