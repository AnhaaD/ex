...
class HistoryComponent extends React.Component<Props> {
    ...
    public render() {
        ...
        return (
          <div className={`pg-history-elem ${list.length ? '' : 'pg-history-elem-empty'} ${className}`}>
              {fetching && <Loader />}
        ...
        switch (type) {
            case 'deposits': {
                ...
                return [
                    ...
                    localeDate(created_at, 'historyDate'),
                    currency && currency.toUpperCase(),
                    wallet ? <Decimal key={txid} fixed={wallet.fixed}>{amount}</Decimal> : 0,
...
export const HistoryElement = injectIntl(connect(mapStateToProps, mapDispatchToProps)(HistoryComponent));


// WEBPACK FOOTER //
// src/drone/src/src/custom/containers/HistoryElement/index.tsx
